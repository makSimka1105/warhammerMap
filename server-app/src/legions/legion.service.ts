...
export class LegionService{
    ...
    async delete(id:ObjectId | string): Promise<{id :string|null,icon:string|void}>{
        ...
        try {
            ...
            const deletedLinks= await Promise.all(deleted.planets.map(async link=>{
    ...
    async updateMain(id: ObjectId | string, dto:Partial<CreateLegionDto>): Promise<Legion | null> {
        handleInvalidIdError(id);
        try {
            const legion =await this.legionModel.findById(id).exec()
            handleObjNotFound(legion, id);
            const oldFileName=(legion&& legion.icon)?legion.icon:''
            const updatedFileName=this.fileService.updateFile(oldFileName,dto.icon)
